...
public class UserServicePostgres implements UserService {
    ...
    public void addUser(User user) {
        String insertUserString = "INSERT INTO \"smartHome_user\" (firstName, lastName, email, password, pat, isVerified) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public User getUserByEmail(String email) {
        String query = "SELECT * FROM \"smartHome_user\" WHERE email = ?";
        User user = null;
    ...
    public boolean deleteUser(String email) {
        String query = "DELETE FROM \"smartHome_user\" WHERE email = ?";
        try (Connection connection = dataSource.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query)) {
    ...
    public User getUserById(int userID) {
        String query = "SELECT * FROM \"smartHome_user\" WHERE userID = ?";
        User user = null;
    ...
    public boolean updateUser(User user) {
        String query = "UPDATE \"smartHome_user\" SET firstName = ?, lastName = ?, email = ?, password = ?, pat = ?, isVerified = ? WHERE userID = ?";
        try (Connection connection = dataSource.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query)) {
    ...
    public boolean verifyUser(String email) {
        String query = "UPDATE \"smartHome_user\" SET isVerified = true WHERE email = ?";
        try (Connection connection = dataSource.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            preparedStatement.setString(1, email);
            return preparedStatement.executeUpdate() > 0;
